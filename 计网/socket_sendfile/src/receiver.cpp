...
int main() {
    ...
    while (true) {
        ...
        // 接收来自发送端的数据包或控制包
        recvfrom(sock, &pkt, sizeof(pkt), 0, (sockaddr *)&client, &len);
        ...
        if (pkt.type == PKT_SYN) {
            ...
            sendto(sock, &synack, sizeof(synack), 0, (sockaddr *)&client, len);
        }
        ...
        // 处理数据包
            ...
            sendto(sock, &ack, sizeof(ack), 0, (sockaddr *)&client, len);
        }
